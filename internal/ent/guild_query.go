...
type GuildQuery struct {
	...
	withAutoRoleRules *AutoRoleRuleQuery
...
// QueryAutoRoleRules chains the current query on the "auto_role_rules" edge.
func (gq *GuildQuery) QueryAutoRoleRules() *AutoRoleRuleQuery {
	query := (&AutoRoleRuleClient{config: gq.config}).Query()
...
func (gq *GuildQuery) Clone() *GuildQuery {
	...
	return &GuildQuery{
		...
		withAutoRoleRules: gq.withAutoRoleRules.Clone(),
...
// WithAutoRoleRules tells the query-builder to eager-load the nodes that are connected to
// the "auto_role_rules" edge. The optional arguments are used to configure the query builder of the edge.
func (gq *GuildQuery) WithAutoRoleRules(opts ...func(*AutoRoleRuleQuery)) *GuildQuery {
	query := (&AutoRoleRuleClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gq.withAutoRoleRules = query
	return gq
}
...
func (gq *GuildQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Guild, error) {
	...
	if query := gq.withAutoRoleRules; query != nil {
		if err := gq.loadAutoRoleRules(ctx, query, nodes,
			func(n *Guild) { n.Edges.AutoRoleRules = []*AutoRoleRule{} },
			func(n *Guild, e *AutoRoleRule) { n.Edges.AutoRoleRules = append(n.Edges.AutoRoleRules, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GuildQuery) loadAutoRoleRules(ctx context.Context, query *AutoRoleRuleQuery, nodes []*Guild, init func(*Guild), assign func(*Guild, *AutoRoleRule)) error {
	...
	query.withFKs = true
	query.Where(predicate.AutoRoleRule(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(guild.AutoRoleRulesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.auto_role_rule_guild
		if fk == nil {
			return fmt.Errorf(`foreign-key "auto_role_rule_guild" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "auto_role_rule_guild" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
